<template>
  <div>
    <nav class="flex items-center space-x-2 text-sm text-muted mb-8 max-w-8xl mx-auto">
      <router-link to="/gateway-list" 
                   class="hover:text-brand transition-colors font-medium flex items-center">
          ...
          <div class="flex flex-col lg:flex-row lg:justify-between lg:items-start space-y-6 lg:space-y-0">
            ...
            <div class="flex items-start space-x-6">
              <div class="relative">
                ...
                <!-- Status Indicator -->
                <div :class="['absolute -top-1 -right-1 w-6 h-6 rounded-full border-2 border-white',
                ...
                <div class="flex flex-wrap items-center gap-3">
                  <span :class="['inline-flex items-center px-3 py-1 rounded-lg text-sm font-semibold',
                    ...
                    <div :class="['w-2 h-2 rounded-full mr-2',
                                 gateway.isActive ? 'bg-green-300' : 'bg-gray-300']"></div>
            ...
            <div class="flex items-center gap-3">
              <button 
                ...
                :class="['px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center',
                        gateway.isActive
                          ? 'bg-white text-gray-700 border border-white/30 hover:bg-white/20'
                          : 'bg-success hover:bg-success-dark text-white',
                        loading.toggle ? 'opacity-50 cursor-not-allowed' : 'hover:scale-105']"
              >
                <div v-if="loading.toggle" class="animate-spin rounded-full h-5 w-5 border-b-2 border-current mr-2"></div>
              ...
              <button 
                @click="$emit('edit')" 
                class="btn btn-secondary text-white border-white/30 hover:bg-white/20"
              >
